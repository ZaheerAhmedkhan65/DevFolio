...
const uploadProfileImage = upload.single('profile_picture');
...
const uploadToCloudinaryMiddleware = async (req, res, next) => {
    try {
        ...
        let folder = 'uploads';
        if (req.file.fieldname === 'post_featured_image') folder = 'posts';
        if (req.file.fieldname === 'project_image') folder = 'projects';
        if (req.file.fieldname === 'profile_picture') folder = 'profiles';
